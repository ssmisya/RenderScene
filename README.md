> **当前状态：返工中，实景验收未通过。** 主工程已整合砖廊双拱中柱、檐口、灯具及市场门组/雨棚/小商铺修订。当前分支为 `codex/sop-reaudit-v2.2.1`。本机双击 **`启动返工预览.command`** 体验本轮更新；原 `启动游戏Demo.command` 和 V2.2.1 ZIP 保留为旧版。当前筛查 75 张候选，只有 2 张满足本批近期目标外景条件；详见 [最新返工与验证](verification/sop_reaudit/README.md)。正式打包仍由 `ACTIVE_REVIEW.json` 阻止。
