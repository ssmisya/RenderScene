...
for mat in bpy.data.materials:
 ...
 for typ,img in images.items():
  if typ=='diff' and mat.name in ['Historic red brick • CC0 scan','Carved terracotta','Terracotta highlight']:
   factors={'Historic red brick • CC0 scan':(1.26,.64,.41),'Carved terracotta':(1.38,.7015,.437),'Terracotta highlight':(1.512,.7686,.4788)}[mat.name]
 ...
 if mat.name=='Cream lamp glass':p.inputs['Emission Color'].default_value=(1,.7,.4,1);p.inputs['Emission Strength'].default_value=.3
