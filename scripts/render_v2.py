...
p=argparse.ArgumentParser();p.add_argument('--mode',choices=['day','night','both'],default='both');p.add_argument('--view',choices=['hero','eye','east','overview','detail','north','close','all'],default='hero');p.add_argument('--width',type=int,default=2400);p.add_argument('--samples',type=int,default=192);p.add_argument('--cpu',action='store_true');a=p.parse_args(sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else [])
R=Path(bpy.data.filepath).parent;D=R/'renders/v2';D.mkdir(exist_ok=True)
try:
...
if a.cpu:device='CPU'
views={'hero':'01_HERO_West_square','eye':'02_EYE_LEVEL_170cm','east':'03_EAST_Facade','overview':'04_MAP_Overview','detail':'05_Brick_detail','north':'06_NORTH_Square','close':'07_Portal_close'}
