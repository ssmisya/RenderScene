...
result={'revision':'2.2.0-demo','source_scene_sha256':hashlib.sha256((R/'Harbin_Sophia_Square.blend').read_bytes()).hexdigest(),'checks':checks,'west_surface_rays':hits,'scene_checks':report,'accuracy_scope':'These checks verify model structure and data, not a 100% real-building match.'}
(R/'validation_v2_2.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n')
