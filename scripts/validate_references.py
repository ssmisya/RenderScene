...
p=argparse.ArgumentParser();p.add_argument('--strict',action='store_true');p.add_argument('--batch',default='v2_2');args=p.parse_args()
R=Path(__file__).resolve().parents[1];D=R/'references'/args.batch
...
(R/'verification'/args.batch/'reference_validation.json').write_text(json.dumps(out,ensure_ascii=False,indent=2)+'\n');print(json.dumps(out,ensure_ascii=False,indent=2))
